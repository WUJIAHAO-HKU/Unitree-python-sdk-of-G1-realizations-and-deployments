...
6. **[Reference Motion Dataset Replacement Guide](#reference-motion-dataset-replacement-guide)**
...
## Reference Motion Dataset Replacement Guide
This section explains how to replace the bundled CR7 reference motion with any other MoCap clip while keeping the ASAP training scripts unchanged.

### ⚡ Quick Reference

#### Required fields (per motion dictionary)

```python
{
    'motion0': {
        'root_trans_offset': np.ndarray,  # [T, 3] root (pelvis) position, not the feet
        'pose_aa': np.ndarray,            # [T, num_joints, 3] axis-angle poses
        'fps': float,                     # sampling rate (30 Hz recommended)
    }
}
```

Only these three fields are mandatory—everything else is reconstructed automatically.

#### CR7 dataset snapshot

- Frames `T`: **119**
- Frame rate `fps`: **30**
- Duration: **3.97 s**
- Root height: z ≈ **0.9 m** (pelvis height, not ground contact)

### 📋 Section Outline

1. [Overview](#overview-1)  
2. [Data Format Requirements](#data-format-requirements)  
3. [Replacement Workflow](#replacement-workflow)  
4. [Configuration Changes](#configuration-changes)  
5. [Training Script Updates](#training-script-updates)  
...
The CR7 clip is referenced in three typical scenarios:

1. **Motion-tracking training** via `robot.motion.motion_file`.
2. **Sim2Real deployments** configured in `sim2real/config/g1_29dof_hist.yaml`.
3. **Standing-pose extraction** for initialization/pose priors.
...
- Standing pose extracted at 2.8 s  
...
#### 1. Motion file structure (`.pkl`)

Each file is a dictionary whose values are single-motion dictionaries. The **required** fields are:

```python
{
    'motion0': {
        'root_trans_offset': np.ndarray,  # [T, 3] world-frame pelvis translation
        'pose_aa': np.ndarray,            # [T, num_joints, 3] axis-angle poses (incl. root)
        'fps': float,                     # sampling rate in Hz (e.g., 30 or 60)
    },
    'motion1': { ... },
    # ...
}
...
{
    'motion0': {
        # Optional 1: controller commands (needed for delta-action training)
        'action': np.ndarray,  # [T, action_dim]

        # Optional 2: SMPL body parameters
        'beta': np.ndarray,    # [10] or [17]

        # Fields below are reconstructed automatically via forward kinematics:
        # 'dof', 'dof_vel', 'root_rot', 'root_lin_vel', 'root_ang_vel', etc.
    }
}
```

#### How the automatic fields are derived

| Auto field | Meaning | Computation |
| --- | --- | --- |
| `root_rot` | Root rotation | Take the root entry of `pose_aa`, convert to quaternion |
| `dof` / `dof_pos` | Robot joint positions | Map `pose_aa` to robot DOFs via FK |
| `dof_vel` | Joint velocities | Frame-to-frame diff of `dof` divided by `dt = 1/fps` |
| `root_lin_vel` | Root linear velocity | Diff of `root_trans_offset` / `dt` |
| `root_ang_vel` | Root angular velocity | Quaternion difference / `dt` |
| `body_vel`, `body_ang_vel`, `rg_pos`, ... | Per-link/marker kinematics | Full-body FK based on the skeleton tree |

As long as `root_trans_offset`, `pose_aa`, and `fps` are consistent, MotionLib reproduces every other quantity that the trainer needs.

#### What is the `action` field and how to log it?

- **Definition**: Per-frame commands issued by a controller or policy (joint targets, torques, end-effector forces, etc.).  
- **When to include**: Only if you plan to reuse those commands later (e.g., delta-action residual learning). Pure motion-tracking datasets can omit it.  
- **How to capture**:
  1. In your logging node (e.g., `listener_deltaa.py`), record the command vector every time you record a MoCap frame.
  2. Stack the commands into a `[T, action_dim]` array and store it under the `action` key.
  3. Feel free to add extra metadata (performer ID, terrain tags, contact labels) if useful downstream.

#### Field descriptions

- **`root_trans_offset`** `[T, 3]`  
  World-frame pelvis position (x, y, z). This is the kinematic root—not the feet. Standing robots typically have z ∈ [0.7 m, 1.0 m].

- **`pose_aa`** `[T, num_joints, 3]`  
  Axis-angle rotations for all joints, including the root at index 0. For `g1_29dof_anneal_23dof`, `num_joints = 24` (1 root + 23 actuated joints).

- **`fps`** `float`  
  Sampling rate used to compute motion duration and interpolation spacing. CR7 uses 30 Hz (119 frames → 3.97 s).
...
motion_data = {
    'motion0': {
        ...
        'pose_aa': np.random.randn(T, num_joints, 3).astype(np.float32),
        'fps': 30.0,
    }
}
...
#### 2. Robot configuration requirements

- The DOF order/count must match `g1_29dof_anneal_23dof`.
- Coordinate frames (world vs. robot) must stay consistent with HumanoidVerse conventions.
- Timing in downstream configs assumes the provided `fps`.

#### 3. Data preprocessing

If your raw motion comes from BVH/FBX/AMASS/etc., convert it into the above schema first. Existing scripts in `scripts/data_process` serve as reference implementations.

---

### Replacement Workflow

#### Step 1 – Prepare the new motion file

1. Place the file under either  
   `humanoidverse/data/motions/g1_29dof_anneal_23dof/` (single file) or  
   `humanoidverse/data/motions/g1_29dof_anneal_23dof/TairanTestbed/singles/` (collection).
2. Confirm it is a `.pkl` following the required schema and joint dimensions.

#### Step 2 – Update training commands
...
- **Interactive script (`train_standing_from_cr7.sh`)**  
  Add a new case option that points to your motion file and desired rewards/project names.

#### Step 3 – Optional config edits

When using the new motion for deployment, edit `sim2real/config/g1_29dof_hist.yaml`:

- `mimic_robot_types`: add `"YourMotion_level1": "g1_29dof_anneal_23dof"`.
- `mimic_models`: map the ID to your exported ONNX checkpoint.
- `start_upper_body_dof_pos`: seed the policy with the first-frame upper-body pose (17 values).
- `motion_length_s`: set the duration (`frames / fps`).

---

### Configuration Changes

#### Example – Adding a “Dance” motion
...
start_upper_body_dof_pos: {
  "Dance_level1": [
    0.0, 0.0, 0.0,        # waist (3)
    0.0, 0.3, 0.0, 1.0,   # left shoulder & elbow (4)
    0.0, 0.0, 0.0,        # left wrist (3)
    0.0, -0.3, 0.0, 1.0,  # right shoulder & elbow (4)
    0.0, 0.0, 0.0         # right wrist (3)
  ],
}
...
Place the corresponding file at  
`humanoidverse/data/motions/g1_29dof_anneal_23dof/dance_motion.pkl` and use it in the usual training command.

---

### Training Script Updates

When editing `train_standing_from_cr7.sh`, add a new `case` branch:

```bash
6)
    MOTION_TYPE="Dance"
    MOTION_FILE="humanoidverse/data/motions/g1_29dof_anneal_23dof/dance_motion.pkl"
    MOTION_DESC="Dance reference motion"
...
Update the prompt text (e.g., `echo "  6) Dance motion"`) so users see the new option.

---

### Validation & Testing

#### 1. Sanity-check the `.pkl`
...
print("=" * 50)
print(f"Total motions: {len(data)}")
for key, motion in data.items():
    ...
    for field in required_fields:
        if field not in motion:
            ...
            print(f"  ✅ {field}: {shape if shape else value}")

    for field in ['action', 'beta']:
        if field in motion:
            print(f"  📦 {field} (optional): {motion[field].shape}")

    if 'pose_aa' in motion and 'root_trans_offset' in motion:
        T_pose = motion['pose_aa'].shape[0]
        T_root = motion['root_trans_offset'].shape[0]
        print("  ⚠️  Frame mismatch" if T_pose != T_root else f"  ✅ Frames: {T_pose}")
        print(f"  ✅ Joints: {motion['pose_aa'].shape[1]}")
print("=" * 50)
```

#### 2. Quick training dry-run
...
Verify that:
- The motion loads without warnings.
- Observation shapes stay consistent.
- Rewards and termination signals behave as expected.

---

### FAQ

**Q1. Do I need to store every field shown in the dataset?**  
No. `root_trans_offset`, `pose_aa`, and `fps` are sufficient. All other kinematic fields are reconstructed.

**Q2. Is the root position the same as the foot position?**  
No. The root corresponds to the pelvis/waist location. Feet positions are derived via FK and usually near ground level, while the root hovers around 0.7–1.0 m.

**Q3. What are the CR7 dataset stats?**  
119 frames sampled at 30 Hz (3.97 s). `pose_aa` has 27 joints (1 root + 26 others); `root_trans_offset` is `[119, 3]`.

**Q4. How should I pick `T` and `fps`?**  
- 30 Hz works for most motions (balanced size/smoothness).  
- 60 Hz is recommended for fast kicks/jumps.  
- Frames = duration × fps (e.g., 4 s × 30 Hz = 120 frames).  
- Avoid fps < 20 or the playback becomes jerky.

**Q5. Joint count does not match the robot.**  
Ensure `pose_aa.shape[1]` equals the expected joint count (24 for `g1_29dof_anneal_23dof`). Extra joints from other datasets must be mapped/trimmed.

**Q6. Playback speed feels off.**  
Double-check `fps`, and ensure `motion_length_s` in YAML matches `frames / fps`.

**Q7. Initial pose mismatch.**  
Update `start_upper_body_dof_pos` with the first-frame pose so the policy initializes near the reference.

**Q8. Coordinate issues.**  
Verify quaternion ordering (xyzw vs. wxyz) and confirm that root translations are in the world frame.

**Q9. How do I prepare the `action` field?**  
If you have a controller output you want to reuse, log it in sync with MoCap frames, stack it into `[T, action_dim]`, and store it under `action`. Skip it if not needed.
...
| Purpose | Path |
| --- | --- |
| Training helper | `train_standing_from_cr7.sh` |
| Sim2Real config | `sim2real/config/g1_29dof_hist.yaml` |
| Motion directory | `humanoidverse/data/motions/g1_29dof_anneal_23dof/` |
| Training entry | `humanoidverse/train_agent.py` |

#### Key parameters

| Parameter | Description | Example |
| --- | --- | --- |
| `robot.motion.motion_file` | Motion path override | `"humanoidverse/data/motions/.../motion.pkl"` |
| `env.config.resample_motion_when_training` | Enable/disable resampling | `True` |
| `project_name` | Logging namespace | `"MotionTracking"` |
| `experiment_name` | Run name | `"YourExperiment"` |

---

### Summary

1. Prepare a `.pkl` motion with `root_trans_offset`, `pose_aa`, and `fps`.  
2. Point `robot.motion.motion_file` (or helper scripts) to the new path.  
3. Update YAML configs and scripts if you plan to deploy or add new menu options.  
4. Sanity-check the dataset and run a quick training dry-run before launching large jobs.

Use the existing CR7 assets as references whenever you need a baseline for file structure, command-line arguments, or deployment configs.
